...
add_filter( 'the_content', 'mercury_remove_empty_p_tags', 15 );

/*  Mercury - Remove empty paragraph tags from shortcodes End  */

function custom_archive_table_of_contents( $atts ) {
    if ( is_archive() && term_description() ) {
        $html = term_description();
        $dom = new DOMDocument;
        ...
        for ($i = 2; $i <= 3; $i++) {
            ...
            foreach ($nodes as $node) {
                $id = sanitize_title_with_dashes($node->textContent);
        ...
        foreach ($headings as $heading) {
            if ($heading['level'] == 2) {
                ...
                if ($prev_level == 2) {
                    $output .= '<ul class="ez-toc-sublist">';
                }
                $output .= sprintf(
                    '<li><a href="#%2$s">%3$s</a></li>',
                    $heading['level'],
...
add_shortcode( 'custom-archive-toc', 'custom_archive_table_of_contents' );

function custom_term_description_with_ids() {
    if ( is_archive() && term_description() ) {
        $html = term_description();
        $dom = new DOMDocument;
        @$dom->loadHTML(mb_convert_encoding($html, 'HTML-ENTITIES', 'UTF-8'));
...
function disable_hobcore_wp_faq_schema_on_archive() {
    if (is_archive()) {
        remove_action('wp_head', 'hobcore_wp_faq_schema_add_header_to_post');
    }
}
