...
function aces_games_settings_init() {
    ...
    add_settings_field(
        'aces_game_rating_stars_number',
        esc_html__( 'The number of stars', 'aces' ),
        'aces_game_rating_stars_number_callback',
        'aces_games_tab',
        'aces_games_tab_other_settings' 
    );
    register_setting( 'aces_games_tab', 'aces_game_rating_stars_number', 'esc_attr');

    function aces_game_rating_stars_number_callback() {
 
        $options = get_option( 'aces_game_rating_stars_number' );
        $number_values = array('5', '6', '7', '8', '9', '10');
        ?>
        <select id="aces_game_rating_stars_number" name="aces_game_rating_stars_number">
    ...
    /*  --- The title "Read Review" button of a game ---  */

    add_settings_field(
        'games_read_review_title',
        esc_html__( 'The global title of the &quot;Read Review&quot; button', 'aces' ),
        'aces_games_textfield_read_review_title_callback',
        'aces_games_tab',
        'aces_games_tab_other_settings',
        array(
            'id' => 'games_read_review_title', 
            'option_name' => 'games_read_review_title'
        )  
    );
    register_setting( 'aces_games_tab', 'games_read_review_title', 'esc_attr');

    function aces_games_textfield_read_review_title_callback($args) {
        $option = esc_attr(get_option($args['option_name']));
        $id = $args['id'];
        $option_name = $args['option_name'];
        ?>
...
add_action( 'admin_init', 'aces_games_settings_init' );